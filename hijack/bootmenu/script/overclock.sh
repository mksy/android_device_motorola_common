...
param_safe()
{
  ...
  export ond_sampling_rate=46875
  export ond_scaling_min_freq=300000
}
...
set_scaling()
{
  case "$scaling" in
    ...
    "7" )
      ...
      echo "smartassv2" > $SCALING_GOVERNOR
    ;;
    "8" )
      if [ $load_all -eq 0 ]; then
         insmod $MODULE_DIR/symsearch.ko
...
if [ $enable -eq 1 ]; then
  ...
  echo $ond_scaling_min_freq > /sys/devices/system/cpu/cpu0/cpufreq/scaling_min_freq
fi
